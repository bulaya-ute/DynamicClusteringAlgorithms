...
def generate_clusters(num_clusters, avg_points_per_cluster, width, height, min_distance, avg_num_outliers):
    ...
    for _ in range(num_clusters):
        ...
        num_points = np.random.randint(max(1, avg_points_per_cluster - 2), avg_points_per_cluster + 3)
    ...
    # Generate outliers
    num_outliers = np.random.randint(max(0, avg_num_outliers - 2), avg_num_outliers + 3)
...
def save_data_to_json(points, width, height, filename):
    """
...
save_data_to_json(points, width, height, 'datasets/dataset1.json')
